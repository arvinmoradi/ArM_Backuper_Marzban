...
function restore {
    clear
    echo "Starting restore process..." # نمایش فقط برای شروع
    ...
    if ls ${BACKUP_PREFIX}*.tar.gz_part* 1> /dev/null 2>&1; then
        ...
        rm -f ${BACKUP_PREFIX}*.tar.gz_part*  # Remove parts after reassembling
        ...
        echo "Error: No DB backup file or parts found!" # فقط در صورت بروز خطا
        read -p "Press any key to return to the main menu..." -n1 -s
        return
    fi
    ...
    # Update progress
    current_step=$((current_step + 1))
    update_progress $((current_step * 100 / TOTAL_STEPS))  # Update progress

    # Step 2: Delete previous contents from the target directory
    rm -rf "$DIR_PATH_1"/*
    if [ $? -ne 0 ]; then
        echo "Failed to delete contents of $DIR_PATH_1." # فقط در صورت بروز خطا
        read -p "Press any key to return to the main menu..." -n1 -s
        return
    fi
    ...
    if [ $? -ne 0 ]; then
        echo "Failed to move backup to $DIR_PATH_1!" # فقط در صورت بروز خطا
        read -p "Press any key to return to the main menu..." -n1 -s
        return
    fi
    ...
    if [ $? -ne 0 ]; then
        echo "Failed to extract the backup!" # فقط در صورت بروز خطا
        read -p "Press any key to return to the main menu..." -n1 -s
        return
    fi
    ...
    if [ $? -ne 0 ]; then
        echo "Failed to delete the backup archive!" # فقط در صورت بروز خطا
        read -p "Press any key to return to the main menu..." -n1 -s
        return
    fi
    ...
    if [ -z "$ARCHIVE_OPT" ]; then
        echo "Error: OPT backup archive not found!" # فقط در صورت بروز خطا
        read -p "Press any key to return to the main menu..." -n1 -s
        return
    fi
    ...
    if [[ "$ARCHIVE_OPT" == *.tar.gz ]]; then
        ...
        echo "Error: Unsupported archive format!" # فقط در صورت بروز خطا
        read -p "Press any key to return to the main menu..." -n1 -s
        return
    fi
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "Failed to restore the OPT files!" # فقط در صورت بروز خطا
        read -p "Press any key to return to the main menu..." -n1 -s
        return
    fi
    ...
    echo -e "\e[1;32mRestore process completed successfully!\e[0m" 
    read -p "Press any key to return to the main menu..." -n1 -s
}
